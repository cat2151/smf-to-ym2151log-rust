...
// Try to load MML WASM module (optional)
async function checkMMLWasm(): Promise<void> {
    try {
        ...
        console.log('MML WASM module not available (this is optional):', (error as Error).message);
...
// Tab switching
function switchTab(tabName: string, clickEvent: Event): void {
    // Hide all tabs
...
function setupEventListeners(): void {
    ...
    document.querySelectorAll('.tab-button').forEach(button => {
        button.addEventListener('click', (event) => {
            const tabName = button.textContent?.includes('MIDI') ? 'midi' : 'mml';
