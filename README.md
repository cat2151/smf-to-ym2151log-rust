...
**A Rust implementation to convert Standard MIDI Files (SMF) into YM2151 register write logs (JSON format)**

## WIP

Currently, it only supports converting basic note events (like C-D-E) into a minimal JSON format.

More advanced features are planned for future implementation.

Frequent breaking changes are expected.

## Purpose

- As a library, it is used by `cat-play-mml`. This is a Rust library crate for native applications (Usage Method 1).
- As a library, it is planned to be used by `web-ym2151`. This is a WASM library for browsers (Usage Method 2).
...
Before conversion, the MIDI file is analyzed to measure the maximum polyphony count (number of concurrent notes) for each MIDI channel by tracking overlapping note events.
...
- MIDI channels with higher polyphony counts receive multiple YM2151 channels.
- Example: If MIDI ch0 requires 3-note polyphony and MIDI ch1 requires 1-note polyphony:
  - MIDI ch0 gets YM2151 ch0, ch1, and ch2 (3 channels).
...
- **Reason**: Drums often have multiple note-on events on the same tick. Because the YM2151 processes channels sequentially and requires specific register access cycles, assigning drums to channel 0 ensures they are triggered first, which improves sound quality.
...
- If the total polyphony count of all MIDI channels exceeds 8, overflow notes use the last assigned channel.
...
It converts Standard MIDI Files (SMF) into YM2151 FM sound chip register write logs in JSON format.
...
-   **WebAssembly (WASM) Compatibility**: Can be run in web browsers via WASM (see [WASM_USAGE.md](WASM_USAGE.md)).
...
A complete demo is available in `index.html`, and a minimal library usage demo is available in `demo-library/`.
...
Each tone file defines YM2151 register writes to set up the FM synthesis parameters. For detailed format documentation and examples, please refer to [`tones/README.md`](tones/README.md).
...
-   [ym2151-zig-cc](https://github.com/cat2151/ym2151-zig-cc): The specification source for the output JSON format.
